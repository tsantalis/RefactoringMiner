...
/**
 * Implementation of {@link CacheStream} that provides support for lazily
 * distributing stream methods to appropriate
 * nodes
 * 
...
public class DistributedCacheStream<R> extends AbstractCacheStream<R, Stream<R>, Consumer<? super R>>
    ...
    // This is a hack to allow for cast to work properly, since Java doesn't work as
    // well with nested generics
    protected static Supplier<CacheStream<CacheEntry>> supplierStreamCast(Supplier supplier) {
        return supplier;
    }

    /**
     * Standard constructor requiring all pertinent information to properly utilize
     * a distributed cache stream
     * 
     * @param localAddress         the local address for this node
     * @param parallel             whether or not this stream is parallel
     * @param dm                   the distribution manager to find out what keys
     *                             map where
     * @param supplier             a supplier of local cache stream instances.
     * @param csm                  manager that handles sending out messages to
     *                             other nodes
     ...
     * @param executor             executor to be used for certain operations that
     *                             require async processing (ie. iterator)
     */
    ...
    /**
     * Constructor that also allows a simple map method to be inserted first to
     * change to another type. This is
     * important because the {@link CacheStream#map(Function)} currently doesn't
     * return a {@link CacheStream}. If this
     ...
     * @param executor             executor to be used for certain operations that
     *                             require async processing (ie. iterator)
     * @param function             initial function to apply to the stream to change
     *                             the type
     */
    public <K, V> DistributedCacheStream(Address localAddress, boolean parallel, DistributionManager dm,
            Supplier<CacheStream<CacheEntry<K, V>>> supplier, ClusterStreamManager csm, boolean includeLoader,
    ...
    /**
     * This constructor is to be used only when a user calls a map or flat map
     * method changing back to a regular
     * Stream from an IntStream, DoubleStream etc.
     * 
     * @param other other instance of {@link AbstractCacheStream} to copy details
     *              from
     */
    ...
    public <U> U reduce(U identity, BiFunction<U, ? super R, U> accumulator, BinaryOperator<U> combiner) {
        return performOperation(TerminalFunctions.reduceFunction(identity, accumulator, combiner), true, combiner,
                null);
    }

    /**
     * {@inheritDoc}
     * Note: this method doesn't pay attention to ordering constraints and any
     * sorting performed on the stream will
     * be ignored by this terminal operator. If you wish to have an ordered
     * collector use the
     * {@link DistributedCacheStream#collect(Collector)} method making sure the
     * {@link java.util.stream.Collector.Characteristics#UNORDERED} property is not
     * set.
     * 
     * @param supplier
     * @param accumulator
    ...
    public <R1, A> R1 collect(Collector<? super R, A, R1> collector) {
        if (sorted) {
            // If we have a sorted map then we only have to worry about it being sorted if
            // the collector is not
            // unordered
            sorted = !collector.characteristics().contains(Collector.Characteristics.UNORDERED);
        }
        // If it is not an identify finish we have to prevent the remote finisher, and
        // apply locally only after
    ...
    Iterator<R> remoteIterator() {
        ...
        if (rehashAware) {
            ...
            NoMapIteratorOperation<?, R> op = new NoMapIteratorOperation<>(intermediateOperations,
                    supplierForSegments(ch,
    ...
    static class HandOffConsumer<R> implements Consumer<R> {
        ...
        public void accept(R rs) {
            // TODO: we don't awake people if they are waiting until we fill up the queue or
            // process retrieves all values
            ...
            if (!queue.offer(rs)) {
                if (!completed.get()) {
                    ...
                    while (!completed.get()) {
                        // We keep trying to offer the value until it takes it. In this case we check
                        // the completed after
     ...
     */
    protected static CacheStream.SegmentCompletionListener composeWithExceptions(
            CacheStream.SegmentCompletionListener a,
