...
public abstract class AbstractCodeMapping {
	...
	public void temporaryVariableAssignment(Set<Refactoring> refactorings, boolean insideExtractedOrInlineMethod) {
		if(this instanceof LeafMapping && getFragment1() instanceof AbstractExpression
			...
			if(variableDeclarations.size() == 1 && validReplacements) {
				...
				ExtractVariableRefactoring ref = new ExtractVariableRefactoring(variableDeclaration, operation1, operation2, insideExtractedOrInlineMethod);
	...
	public void temporaryVariableAssignment(AbstractCodeFragment statement,
			List<? extends AbstractCodeFragment> nonMappedLeavesT2, UMLClassBaseDiff classDiff, boolean insideExtractedOrInlineMethod) {
					...
					if(replacement.getBefore().endsWith(suffixAfter)) {
						...
						if(initializer != null) {
							if(initializer.toString().equals(prefixBefore) ||
								...
								ExtractVariableRefactoring ref = new ExtractVariableRefactoring(declaration, operation1, operation2, insideExtractedOrInlineMethod);
								processExtractVariableRefactoring(ref, refactorings);
	...
	public void inlinedVariableAssignment(AbstractCodeFragment statement,
			List<? extends AbstractCodeFragment> nonMappedLeavesT2, boolean insideExtractedOrInlineMethod) {
		for(VariableDeclaration declaration : statement.getVariableDeclarations()) {
			for(Replacement replacement : getReplacements()) {
				String variableName = declaration.getVariableName();
				AbstractExpression initializer = declaration.getInitializer();
					...
					if(replacement.getAfter().endsWith(suffixBefore)) {
						...
						if(initializer != null) {
							if(initializer.toString().equals(prefixAfter) ||
								...
								InlineVariableRefactoring ref = new InlineVariableRefactoring(declaration, operation1, operation2, insideExtractedOrInlineMethod);
