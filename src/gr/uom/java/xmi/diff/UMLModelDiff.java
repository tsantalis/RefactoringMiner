...
public class UMLModelDiff {
    ...
    public Map<NameSpace, Set<String>> getNamespaceForImports(GlobalContext gc, Set<String> classNames) {
        ...
        if (map.containsKey(Internal))
            map.get(Internal).addAll(streamOriginalClasses().map(UMLClass::getPackageName).collect(toList()));
        return map;
    }

    private NameSpace getNameSpaceForImportStmt(String importStmt, GlobalContext gc, Set<String> classNames) {
        return gc.getAllInternalPackages()
                .filter(p -> classNames.stream().anyMatch(c -> c.contains(p)))
    ...
    public CodeStatistics getMatchedCodeStatistic(GlobalContext gc, Set<String> classNames, Set<String> forTypes) {
        CodeStatistics cs = streamOriginalClasses()
                .filter(x -> classNames.stream().anyMatch(c -> c.contains(x.getName())))
                .map(x -> x.getMatchedCodeStatistic(gc, forTypes))
