...
public abstract class AbstractXYItemRenderer extends AbstractRenderer
    ...
    public void drawDomainMarker(Graphics2D g2, XYPlot plot, 
        ...
        drawMarker(marker, domainAxis, plot, dataArea, g2, () -> plot.getDomainAxisEdge(),
						PlotOrientation.VERTICAL, PlotOrientation.HORIZONTAL,
						(PlotOrientation orientation, Rectangle2D rect,
								RectangleAnchor anchor) -> calculateDomainMarkerTextAnchorPoint(g2, orientation,
										dataArea, rect, marker.getLabelOffset(),
    ...
    public void drawRangeMarker(Graphics2D g2, XYPlot plot, ValueAxis rangeAxis,
        ...
        drawMarker(marker, rangeAxis, plot, dataArea, g2, () -> plot.getRangeAxisEdge(),
						PlotOrientation.HORIZONTAL, PlotOrientation.VERTICAL,
						(PlotOrientation orientation, Rectangle2D rect,
								RectangleAnchor anchor) -> calculateRangeMarkerTextAnchorPoint(g2, orientation,
										dataArea, rect, marker.getLabelOffset(),
										LengthAdjustmentType.EXPAND, anchor),
						(PlotOrientation orientation, Rectangle2D rect,
								RectangleAnchor anchor) -> calculateRangeMarkerTextAnchorPoint(g2, orientation,
	...
	private void drawMarker(Marker marker, ValueAxis domainAxis, XYPlot plot, Rectangle2D dataArea, Graphics2D g2,
			Supplier<RectangleEdge> edgeSupplier, PlotOrientation po1, PlotOrientation po2, CoordinateCalculation coordinateCalc1,
			CoordinateCalculation coordinateCalc2) {
		if (marker instanceof ValueMarker) {
			...
			double v = domainAxis.valueToJava2D(value, dataArea, edgeSupplier.get());
			PlotOrientation orientation = plot.getOrientation();
			Line2D line = null;
			if (orientation == po1) {
				line = new Line2D.Double(v, dataArea.getMinY(), v, dataArea.getMaxY());
			} else if (orientation == po2) {
			...
			if (label != null) {
				...
				Point2D coordinates = coordinateCalc1.apply(orientation, line.getBounds2D(), anchor);
			...
			double start2d = domainAxis.valueToJava2D(start, dataArea, edgeSupplier.get());
			...
			if (label != null) {
				...
				g2.setFont(labelFont);
				g2.setPaint(marker.getLabelPaint());
				Point2D coordinates = coordinateCalc2.apply(orientation, rect, anchor);
				TextUtilities.drawAlignedString(label, g2, (float) coordinates.getX(), (float) coordinates.getY(),
						marker.getLabelTextAnchor());
			}
			g2.setComposite(originalComposite);
