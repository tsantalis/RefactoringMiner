...
public class ExtendedMultiMappingStore extends MultiMappingStore implements Iterable<Mapping> {
    ...
    private boolean sameFile(Tree t1, Tree t2) {
        ...
        if(t1_p == null && t2_p == null) {
            t1_p = TreeUtilFunctions.getParentUntilType(t1, LANG1.PROGRAM);
            t2_p = TreeUtilFunctions.getParentUntilType(t2, LANG2.PROGRAM);
        }
        if(t1_p == null && t2_p == null) {
            t1_p = TreeUtilFunctions.getParentUntilType(t1, LANG1.TRANSLATION_UNIT);
            t2_p = TreeUtilFunctions.getParentUntilType(t2, LANG2.TRANSLATION_UNIT);
        }
