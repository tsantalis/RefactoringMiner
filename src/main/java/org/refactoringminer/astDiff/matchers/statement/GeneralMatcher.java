...
public class GeneralMatcher extends BasicTreeMatcher implements TreeMatcher {
    ...
    public GeneralMatcher(AbstractCodeFragment st1, AbstractCodeFragment st2, Constants LANG1, Constants LANG2) {
        super(LANG1, LANG2);
        this.st1 = st1;
        this.st2 = st2;
    }
    ...
    public void match(Tree src, Tree dst, ExtendedMultiMappingStore mappingStore) {
        ...
        if ((st1 instanceof CompositeStatementObject) &&  (st2 instanceof CompositeStatementObject)) {
            new CompositeMatcher((CompositeStatementObject) st1, (CompositeStatementObject) st2, LANG1, LANG2)
        ...
        if (!(st1 instanceof CompositeStatementObject) &&
            ...
            for (AbstractExpression expression : fragment2.getExpressions()) {
                Tree dstExpTree = TreeUtilFunctions.findByLocationInfo(dst, expression.getLocationInfo(), LANG2);
                new LeafMatcher(LANG1, LANG2).match(src,dstExpTree,mappingStore);
            }
            ...
            for (AbstractExpression expression : fragment1.getExpressions()) {
                Tree srcExpTree = TreeUtilFunctions.findByLocationInfo(src, expression.getLocationInfo(), LANG1);
                new LeafMatcher(LANG1, LANG2).match(srcExpTree,dst,mappingStore);
            }
        }
        else {
            new LeafMatcher(LANG1, LANG2).match(src,dst,mappingStore);
        }
